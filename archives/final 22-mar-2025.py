...
def geocode_addresses(addresses):
    ...
    for address in addresses:
        try:
            location = geolocator.geocode(address + ", Bolsover, UK")
            if location:
                locations.append((location.latitude, location.longitude))
            else:
                locations.append((None, None))  # Handle cases where geocoding fails
        except Exception as e:
...
def aggregated_analysis_page():
    ...
    fig = create_bolsover_map(df_letters)
    st.plotly_chart(fig, use_container_width=True)

    # 9) Location Visualization
    st.subheader("📍 Geographic Issue Distribution")

    # Process data with locations
    df_locations = process_uploaded_data(st.session_state.uploaded_files_texts)
    ...
    if not df_locations.empty:
        ...
        fig = px.scatter_mapbox(
            ...
            height=600,
            color_discrete_map={
                "Local Problem": "#FF0000",
                "New Initiative": "#00FF00"
            }
        )
        ...
        st.warning("No geographic data could be extracted from the documents")
