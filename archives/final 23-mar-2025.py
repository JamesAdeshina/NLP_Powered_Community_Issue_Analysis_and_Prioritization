        ...
        if df_agg.empty or df_agg["lat"].isnull().all() or df_agg["lon"].isnull().all():
            ...
            if df_sentiment.empty:
                ...
                df_sentiment["color"] = df_sentiment["sentiment"].apply(
                    lambda x: [0, 255, 0] if x == "POSITIVE" else [255, 0, 0])
                ...
                view_state = pdk.ViewState(latitude=df_sentiment["lat"].mean(), longitude=df_sentiment["lon"].mean(),
                                           zoom=10)
    ...
    with tab2:
        ...
        if df_agg.empty or df_agg["lat"].isnull().all() or df_agg["lon"].isnull().all():
            ...
            if df_category.empty:
                ...
                view_state = pdk.ViewState(latitude=df_category["lat"].mean(), longitude=df_category["lon"].mean(),
                                           zoom=10)
                deck = pdk.Deck(layers=[layer], initial_view_state=view_state,
    ...
    with tab3:
        ...
        if df_agg.empty or df_agg["lat"].isnull().all() or df_agg["lon"].isnull().all():
            ...
            if df_topic.empty:
                ...
                view_state = pdk.ViewState(latitude=df_topic["lat"].mean(), longitude=df_topic["lon"].mean(), zoom=10)
                deck = pdk.Deck(layers=[layer], initial_view_state=view_state,
    ...
    with tab4:
        ...
        if df_agg.empty or df_agg["lat"].isnull().all() or df_agg["lon"].isnull().all():
            ...
            if df_issue.empty:
                ...
                view_state = pdk.ViewState(latitude=df_issue["lat"].mean(), longitude=df_issue["lon"].mean(), zoom=10)
                deck = pdk.Deck(layers=[layer], initial_view_state=view_state,
