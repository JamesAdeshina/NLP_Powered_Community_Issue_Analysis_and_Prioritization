...
# Consider lazy importing these inside functions that require them.
...
# ------------------ Helper: Compute Topic for Individual Letter ------------------
# Create a separate set of candidate labels for topic detection.
...
# Update the dynamic_topic_label() function to use the new list:
def dynamic_topic_label(keywords):
    result = zero_shot_classifier(keywords, candidate_labels_topic)
    best_label = result["labels"][0]
    return best_label


# In your compute_topic() function (for individual letters), it remains the same:
def compute_topic(text, top_n=5):
    # Use RAKE to extract key phrases from the text.
    rake_extractor = Rake()  # RAKE uses a default stopword list for English
    rake_extractor.extract_keywords_from_text(text)
    # Get the top-ranked phrases
    ranked_phrases = rake_extractor.get_ranked_phrases()
    # Use the top_n phrases as keywords (if available)
    top_terms = ranked_phrases[:top_n] if len(ranked_phrases) >= top_n else ranked_phrases
    keyword_str = ", ".join(top_terms)
    # Pass the key phrases to your zero-shot classifier with an expanded candidate list
...
# (Using the candidate_labels defined earlier for classification remains unchanged.)
...
def main():
    ...
    with tabs[1]:
        ...
        if "input_text" not in st.session_state or not st.session_state.input_text:
            ...
            # Compute topic for the individual letter.
            topic_label, top_keywords = compute_topic(letter_clean)
            st.subheader("Topic")
            # st.write(f"Topic: **{topic_label}** (Top Keywords: {top_keywords})")
            st.write(f"Topic: **{topic_label}**")

            # Query input appears at the top of the Results tab.
