...
def unsupervised_classification(texts, num_clusters=2):
    # Use n-grams (unigrams and bigrams) for richer features.
...
def dynamic_label_clusters(vectorizer, kmeans):
    ...
    for i in range(kmeans.n_clusters):
        ...
        # Debug print can be uncommented for troubleshooting:
        # print(f"Cluster {i}: Keywords: {keyword_str} => Classified as: {best_label}")
...
# Cache the abstractive summarizer so it's loaded only once.
...
def query_based_summarization(text, query, threshold=0.1, top_n=2):
    ...
    # Refine query handling: for action-oriented queries, lower threshold and filter for action words.
    action_indicators = ["urge", "request", "increase", "arrange", "immediate", "control", "measure", "action", "implement", "improve", "take"]
...
# ------------------ Sentiment Analysis ------------------
# Cache the transformer sentiment pipeline.
...
def sentiment_analysis(text):
    ...
    # Use transformer output primarily, but override to Negative if VADER is strongly negative.
...
def main():
    ...
    print("Query-based Summary 1 (for query 'air pollution'):", query_based_summarization(sample_text, query="air pollution"))
    ...
    compound_score = sentiment_results["vader_scores"]["compound"]
